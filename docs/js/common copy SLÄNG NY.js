...
function updateCursorVisibility(event) {
  if (!customCursor) return; // <--- skydda mot null
...
function openModal(componentName) {
  console.log("Opening modal:", componentName);
...
appModal.addEventListener("close-popup", () => {
  appModal.close();
});

// Visa bekräftelse i popup-wrapper
appModal.addEventListener("show-confirmation", (e) => {
  const content = e.detail.content;

  // Visa innehållet i popup-wrapper
  appModal.open(content);

  // Lägg till event på OK-knappen för att stänga popupen
  const okBtn = content.querySelector(".confirm");
...
/* document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("storage-cleared", () => {
    updateStars();
...
document.addEventListener("storage-cleared", () => {
  ...
  setTimeout(() => {
    appModal.open(confirmation);

    confirmation
      .querySelector(".confirm")
      .addEventListener("click", () => appModal.close());
  }, 200);
}); */
...
/* document.addEventListener("DOMContentLoaded", () => {
  ...
  if (appModal) {
    appModal.addEventListener("close-popup", () => appModal.close());
  }
  ...
  document.addEventListener("storage-cleared", () => {
    ...
    if (typeof updateTotalStarsForCurrentLanguage === "function")
      updateTotalStarsForCurrentLanguage();
    ...
    if (appModal) {
      ...
      setTimeout(() => {
        ...
        if (confirmBtn) {
          confirmBtn.addEventListener("click", () => appModal.close());
        }
      }, 200);
    }
  });
...
scrollButton.addEventListener("click", () => {
  ...
  if (textPath) {
    ...
    // Pausa när musen klickar på scroll-området
    if (sectionText) {
      sectionText.addEventListener("click", () => {
        isPaused = !isPaused;
      });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!scrollButton || !scrollArea) return;
